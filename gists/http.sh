...
echo "host.consul.example cert"
cat << EOL | ./consul config write -
...
echo "foo.consul.example cert"
cat << EOL | ./consul config write -
kind = "inline-certificate"
name = "foo-consul-example"
private_key = <<EOF
-----BEGIN RSA PRIVATE KEY-----
MIIEpAIBAAKCAQEA0wzZeonUklhOvJ0AxcdDdCTiMwR9tsm/6IGcw9Jm50xVY+qg
5GFg1RWrQaODq7Gjqd/JDUAwtTBnQMs1yt6nbsHe2QhbD4XeqtZ+6fTv1ZpG3k8F
...
echo "other.consul.example cert"
cat << EOL | ./consul config write -
kind = "inline-certificate"
...
echo "Writing conflicted http route config entry"
cat << EOF | ./consul config write -
kind = "http-route"
...
echo "Registering http service two"
cat << EOF > /tmp/service2.hcl
service {
  name = "service-two"
  id   = "service-two"
  port = 9008
...
EOF
./consul services register /tmp/service2.hcl
...
echo "Registering http service proxy two"
cat << EOF > /tmp/proxy2.hcl
service {
  kind = "connect-proxy"
  name = "service-two"
  id   = "service-two"
...
EOF
./consul services register /tmp/proxy2.hcl
...
cat << EOF > /tmp/server2.html
HTTP/1.1 200 GET
Content-Type: text/html; charset=UTF-8

<!doctype html><html><body>server2</body></html>
EOF
...
echo "Running http service two"
ncat -e "/bin/cat /tmp/server2.html" -k -l 9008 &
echo "Running http service two sidecar proxy"
